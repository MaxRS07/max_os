use core::{cmp::Ordering, fmt::Debug, todo};
...
impl FSHeader {
    ...
    fn new(
        ...
        let name = Self::name_to_bytes(name)?;
        Ok(Self {
            name,
...
impl Debug for FSHeader {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let _ = f.write_str("FileHeader { ");
        let _ = f.write_fmt(format_args!("is_directory: {}", self.is_dir()));
        f.write_str(" }")
    }
}
